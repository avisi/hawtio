...
module AsquareConnect {
  ...
  export var log:Logging.Logger = Logger.get("AsquareConnect");
  ...
  export function configureScope($scope, $location, workspace) {
    ...
    $scope.hasLocalMBean = () => {
      return AsquareConnect.hasLocalMBean(workspace);
    };

    $scope.breadcrumbs = [
      {
        content: '<i class=" icon-signin"></i> Remote',
        title: "Connect to a remote AsquareConnect running Jolokia",
  ...
  export function hasLocalMBean(workspace) {
    return workspace.treeContainsDomainAndProperties('hawtio', {type: 'AsquareConnectList'});
  }
